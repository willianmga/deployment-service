...
- JWT_TOKEN_PRIVATE_KEY: A base 64 encoded private RSA key suitable for RS512 signing algorithm
...
Request:
```shell
curl --location --request POST 'http://localhost:8080/v1/services/deploy/8' \
--header 'Authorization: Bearer YOUR_JWT_TOKEN'
```
...
Request:
```shell
curl --location --request GET 'http://localhost:8080/v1/services/8' \
--header 'Authorization: Bearer YOUR_JWT_TOKEN'
```
